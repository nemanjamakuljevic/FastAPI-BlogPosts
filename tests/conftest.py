...
def test_user2(client):
    user_data = {"email": "testic@gmail.com", "password": "testpass"}
    res = client.post("/users/", json = user_data)
    assert res.status_code == 201
...
def test_posts(test_user,test_user2,session):
    ...
    # session.add_all([models.Post(title="first post", content="first content", owner_id=test_user['id']),
    #                 models.Post(title="2nd post", content="2nd content", owner_id=test_user['id']),
    #                 models.Post(title="3rd post", content="3rd content", owner_id=test_user['id'])])
    session.commit()
